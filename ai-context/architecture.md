...
- `src/App.tsx` — initialization, active-database selection, route selection, live state refresh, cross-tab broadcasts, and mutation callbacks.
- `src/data/database.ts` — database-library catalogue plus common browser/native SQLite adapters.
...
- empty hash / `#/` → control centre
- `#/projector` → public projector leaderboard
...
Fruits of the Spirit uses the existing `add` event path rather than a separate score type or table. Its nine fixed values and 5-point amount live in `src/config.ts`; the organizer writes the selected fruit into the immutable `reason` snapshot. `TransferMoment` recognises that reason prefix and gives it the same extended projector treatment as other camp-wide callouts.

Organizer data tools call typed ledger methods directly rather than dispatching a custom window event. They always target the database selected at the control centre: backup/import replaces only that database, while CSV and browser SQLite exports report that database. Browser downloads attach a temporary anchor to the document and defer object-URL revocation for WebKit compatibility. The database adapter exposes read-only byte export in the browser; native builds use their existing on-disk file.
